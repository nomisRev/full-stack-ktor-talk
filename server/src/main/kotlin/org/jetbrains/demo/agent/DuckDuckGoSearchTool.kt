...
fun main() = runBlocking<Unit> {
    HttpClient {
        install(ContentNegotiation) {
            json(contentType = ContentType(ContentType.Application.TYPE, "x-javascript"))
        }
    }.use { client ->

        val tool = DuckDuckGoSearchTool(client)
        println(tool.search("Kotlin"))
    }
}
...
class DuckDuckGoSearchTool(
    ...
    suspend fun search(
        ...
        val json = client.get("https://api.duckduckgo.com/") {
            parameter("q", query)
        ...
        val response = JSON.decodeFromJsonElement(DuckDuckGoResponse.serializer(), json)
